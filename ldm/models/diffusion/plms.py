...
class PLMSSampler_Timewise(object):
    def __init__(self, model, schedule="linear", **kwargs):
        super().__init__()
        self.model = model
        self.ddpm_num_timesteps = model.num_timesteps
    ...
    def plms_sampling(self, cond, shape,
        ...
        # Added for debugging
        # Remove when generating evaluation samples
        # torch.manual_seed(0)
        ...
        if self.seed1 is not None and self.seed3 is not None:
            if self.count % 2 == 0:
                torch.set_rng_state(self.seed1)
                torch.cuda.set_rng_state(self.seed2)
            else:
                torch.set_rng_state(self.seed3)
                torch.cuda.set_rng_state(self.seed4)
        else:
            if self.seed3 is None:
                if self.seed1 is None:
                    self.seed1 = torch.random.get_rng_state()
                    self.seed2 = torch.cuda.random.get_rng_state()
                else:
                    self.seed3 = torch.random.get_rng_state()
                    self.seed4 = torch.cuda.random.get_rng_state()
            if self.seed3 is None:
                torch.set_rng_state(self.seed1)
                torch.cuda.set_rng_state(self.seed2)
            else:
