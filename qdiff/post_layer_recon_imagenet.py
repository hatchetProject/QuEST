...
def pd_optimize_timewise(model, cali_data, opt, logger, iters: int = 20000, timesteps: list=None, outpath: str=None,):
    ...
    # optimizer_alpha = torch.optim.Adam(opt_params_alpha, lr=1e-4)
    # scheduler_w = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_w, T_max=1000, eta_min=0.)
    scheduler_w = None
    # scheduler_alpha = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_alpha, T_max=1000, eta_min=0.)
    ...
    # layer_list = ["ff.net", "to_q"]
    layer_list = ["ff.net", "to_q", "attn1.to_k", "attn1.to_v"]
    save_folder = "output/fp_activations_timewise2"
    ...
    for epoch in range(20):
        for t in timesteps:
            ...
            # scheduler_a = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_a, T_max=30, eta_min=0.)
            scheduler_a = None
            ...
            before_step = {name: p.clone() for name, p in model.named_parameters()}
            model.train()
            ...
            for j in range(num):
                ...
                # print(f"Time for iteration {j} update: ", end_time_1 - start_time)
...
def pd_optimize_timeembed(model, cali_data, opt, logger, iters: int = 20000, timesteps: list=None, outpath: str=None,):
    ...
    for name, module in model.named_modules():
        if isinstance(module, (QuantModule)):
            if "emb_layer" in name:
                ...
                # opt_params_alpha += [module.weight_quantizer.alpha]
    optimizer_w = torch.optim.Adam(opt_params_w, lr=1e-5)
    # scheduler_w = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_w, T_max=1000, eta_min=0.)
    cali_data = (cali_data[0].cpu(), cali_data[1].cpu(), cali_data[2].cpu())
    # Get intermediate activations
    ...
    for t in timesteps:
        ...
        # scheduler_a = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_a, T_max=100, eta_min=0.)
        # scheduler_w = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_w, T_max=100, eta_min=0.)
        scheduler_a = None
        scheduler_w = None
        optimizer_a.zero_grad()
        optimizer_w.zero_grad()
        ...
        for epoch in range(300):
            ...
            after_step = {name: p for name, p in model.named_parameters()}
            count = 0
        for handle in hook_handles:
            handle.remove()
        torch.cuda.empty_cache()
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.rec_loss == 'mse':
            ...
            # rec_loss = new_lp_loss(pred, tgt, p=self.p)
            # rec_loss = new_loss_1(pred, tgt, p=self.p)
            # rec_loss = new_loss_2(pred, tgt, p=1.0, k=2)
        else:
