...
from qdiff.quant_layer import QuantModule, UniformAffineQuantizer, TimewiseUniformQuantizer, ActUniformQuantizer
    ...
    if quant_act:       
            ...
            if isinstance(m, AdaRoundQuantizer):
                ...
                if m.zero_point is not None:
                    if not torch.is_tensor(m.zero_point):
                        ...
                        m.zero_point = nn.Parameter(m.zero_point)
            elif isinstance(m, TimewiseUniformQuantizer):
                ...
                m.delta_list = nn.Parameter(m.delta_list)
        
        qnn.set_timestep(timesteps[0])
        for m in qnn.model.modules():
            if hasattr(m, "current_delta"):
                delattr(m, "current_delta")
                # print(f"Deleted current_delta from {m}")
        ckpt = torch.load(ckpt_path, map_location='cpu')
        keys = [key for key in ckpt.keys() if "current_delta" in key]
        for key in keys:
            del ckpt[key]

        # for key in ckpt.keys():
        #     if "current_delta" in key:
        #         ckpt[key] = ckpt[key].unsqueeze(0)
        ...
        for m in qnn.model.modules():
            if isinstance(m, AdaRoundQuantizer):
                ...
                if m.zero_point_list is not None:
                    zero_data = m.zero_point_list.data
                    delattr(m, "zero_point_list")
                    # assert(zero_data.int() == zero_data)
                    m.zero_point_list = zero_data.int()
        qnn.load_dict_params()
